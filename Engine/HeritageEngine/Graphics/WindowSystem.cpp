...
void WindowSystem::setMode(GLFWwindow* window, WindowMode mode, int desiredW, int desiredH, int desiredRefresh)
{
    GLFWmonitor* mon = glfwGetPrimaryMonitor();
    const GLFWvidmode* vid = glfwGetVideoMode(mon);

    if (mode == WindowMode::Windowed) {
        glfwSetWindowMonitor(window, nullptr, m_savedX, m_savedY, m_savedW, m_savedH, GLFW_DONT_CARE);
    }
    else if (mode == WindowMode::Borderless) {
        glfwSetWindowMonitor(window, nullptr, 0, 0, vid->width, vid->height, GLFW_DONT_CARE);
    }
    else { // Exclusive
        int w = (desiredW > 0) ? desiredW : vid->width;
        int h = (desiredH > 0) ? desiredH : vid->height;
        int refresh = (desiredRefresh != GLFW_DONT_CARE) ? desiredRefresh : vid->refreshRate;
        glfwSetWindowMonitor(window, mon, 0, 0, w, h, refresh);
    }
    m_mode = mode;
}
...
void WindowSystem::drawResizeHandles(GLFWwindow* window)
{
    ...
    if (!m_resizing) {
        if (!io.WantCaptureMouse && hover != 0) {
            if ((hover & (1 | 8)) == (1 | 8) || (hover & (2 | 8)) == (2 | 8))
        ...
        // Active resizing
        if ((m_resizeDir & 1) && (m_resizeDir & 8)) setCursor(CursorType::DiagNWSE);
