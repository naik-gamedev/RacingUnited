#include "DynamicSurfaceHydrologyReference.hpp"
...
void DynamicSurfaceHydrologyReference::clear()
{
    m_pages.clear();
    m_stats = {};
...
void DynamicSurfaceHydrologyReference::resetWater()
{
...
DynamicSurfaceHydrologyReference::HydroPage* DynamicSurfaceHydrologyReference::ensurePage(
    ...
    const SurfaceWeatherOutput* initialWeather)
{
    ...
    // OPT02C: locateMutable() calls ensurePage() for every tire contact. Once a
    // page already exists, its cached statistics are maintained by the mutation
    // paths themselves; rescanning 256x256 texels here made every contact O(N).
    if (inserted)
        refreshPageStats(page);
    return &page;
}

bool DynamicSurfaceHydrologyReference::buildStaticPage(
    const DynamicSurfaceSystem& owner,
...
DynamicSurfaceHydrologyReference::LocatedCell DynamicSurfaceHydrologyReference::locateMutable(
    DynamicSurfaceSystem& owner,
...
DynamicSurfaceHydrologyReference::LocatedCell DynamicSurfaceHydrologyReference::locateConst(
    const DynamicSurfaceSystem& owner,
...
std::optional<VirtualPageAddress> DynamicSurfaceHydrologyReference::neighbourPage(
    const DynamicSurfaceSystem& owner,
    const VirtualPageAddress& address,
    int pageDx,
    int pageDz) const
...
bool DynamicSurfaceHydrologyReference::simulateEnvironment(
    HydroPage& page,
...
bool DynamicSurfaceHydrologyReference::simulateFlow(
    DynamicSurfaceSystem& owner,
    HydroPage& page,
    double stepSeconds,
    double pageHz)
...
void DynamicSurfaceHydrologyReference::advance(
    DynamicSurfaceSystem& owner,
    ...
    double deltaTimeSeconds)
{
    ...
    // LIVETRACK10B: even the headless/CPU fallback must obey the same bounded
    // detailed-water ownership rule as production. Static scene chunks may span
    // kilometres, but a 256x256 Hydro page is materialized only when its 100m
    // page rectangle intersects the <=100m union around an active interest
    // source. This prevents the regression/fallback path from quietly rebuilding
    // whole-world detailed Hydro state that production intentionally deleted.
    const double nearRadiusSquared =
        UpdateCadence::puddleVisualRangeM * UpdateCadence::puddleVisualRangeM;
    const bool boundedByInterest = !owner.interestSources().empty();

    if (boundedByInterest)
    {
        for (auto it = m_pages.begin(); it != m_pages.end(); )
        {
            ...
            {
                if (distanceSquaredToPageBounds(source, it->first) <= nearRadiusSquared)
                {
                    nearAnySource = true;
                    break;
                }
    ...
    for (const ChunkAddress chunkAddress : owner.chunkAddresses())
    {
        const DynamicSurfaceChunk* chunk = owner.findChunk(chunkAddress);
        if (!chunk || !chunkHasHydroGeometry(*chunk))
            continue;
...
DynamicSurfaceHydroSample DynamicSurfaceHydrologyReference::sample(
    const DynamicSurfaceSystem& owner,
    const heritage::math::DVec3& globalPosition) const
{
...
DynamicSurfaceHydroTireResult DynamicSurfaceHydrologyReference::applyTireContact(
    DynamicSurfaceSystem& owner,
    const heritage::math::DVec3& globalPosition,
    const DynamicSurfaceHydroTireInput& input)
{
    ...
    {
        const std::uint16_t beforePacked = cell.rgba4;
        std::uint16_t afterPacked = withWaterLevel(beforePacked, finalLevel);
        const int boost = std::clamp(
            static_cast<int>(initialLevel) - static_cast<int>(finalLevel),
            1,
            3);
        afterPacked = withDryLineLevel(
            afterPacked,
            static_cast<std::uint8_t>(std::min<int>(
                15,
                static_cast<int>(dryLineLevel(afterPacked)) + boost)));
        cell.rgba4 = afterPacked;
        updateWaterStatsForCellChange(*located.page, beforePacked, afterPacked);
    ...
    if (displacedVolumeM3 > 0.0 && (dx != 0 || dz != 0))
    {
        ...
        if (targetPage)
        {
            ...
            if (redistributedM3 > 1.0e-12)
            {
                ...
                {
                    const std::uint16_t beforePacked = target.rgba4;
                    const std::uint16_t afterPacked = withWaterLevel(
                        beforePacked, targetLevel);
                    target.rgba4 = afterPacked;
                    updateWaterStatsForCellChange(
                        *targetPage, beforePacked, afterPacked);
                    ++targetPage->revision;
                }
            }
        }
...
bool DynamicSurfaceHydrologyReference::rasterPage(
    const DynamicSurfaceSystem& owner,
    const VirtualPageAddress& requestedAddress,
...
std::uint64_t DynamicSurfaceHydrologyReference::pageRevision(
...
bool DynamicSurfaceHydrologyReference::setUniformWaterDepthForLab(
    DynamicSurfaceSystem& owner,
    double waterDepthM)
{
...
void DynamicSurfaceHydrologyReference::refreshPageStats(HydroPage& page)
{
    page.waterLevelHistogram.fill(0u);
    ...
    for (const StateCell& cell : page.stateCells)
    {
        ...
        ++page.waterLevelHistogram[water];
...
void DynamicSurfaceHydrologyReference::updateWaterStatsForCellChange(
    HydroPage& page,
    std::uint16_t beforePacked,
    std::uint16_t afterPacked)
{
    const std::uint8_t before = waterLevel(beforePacked);
    const std::uint8_t after = waterLevel(afterPacked);
    if (before == after)
        return;

    if (page.waterLevelHistogram[before] > 0u)
        --page.waterLevelHistogram[before];
    ++page.waterLevelHistogram[after];

    if (before == 0u && after != 0u)
        ++page.wetCellCount;
    else if (before != 0u && after == 0u && page.wetCellCount > 0u)
        --page.wetCellCount;

    const double cellAreaM2 =
        kHydroAuthorityTexelPitchM * kHydroAuthorityTexelPitchM;
    page.cachedWaterVolumeM3 +=
        (decodeWaterDepth4Bit(after) - decodeWaterDepth4Bit(before))
        * cellAreaM2;
    page.cachedWaterVolumeM3 = std::max(page.cachedWaterVolumeM3, 0.0);

    page.cachedMaximumWaterDepthM = 0.0;
    for (int level = 15; level >= 1; --level)
    {
        if (page.waterLevelHistogram[static_cast<std::size_t>(level)] != 0u)
        {
            page.cachedMaximumWaterDepthM = decodeWaterDepth4Bit(
...
void DynamicSurfaceHydrologyReference::refreshStats()
{
