...
set "LOG=%DIAGNOSTICS%\step29j2_build_and_run.txt"
...
echo ============================================================
echo Heritage Engine - Step 29J.2 Blender Authoring + Driveable Player Scene
...
for %%F in (
    ...
    echo ERROR: Required Step 29J.2 file is missing:
    echo %%~F
    pause
    exit /b 1
)
...
findstr /C:"DRIVEABLE PLAYER SCENE - STEP 29J.2" "%MODULE%\Scripts\UI\Prototype\ScenePanel.lua" >nul || goto :layout_failed
...
echo Generating Step 29J.2 build identity...
powershell -NoProfile -ExecutionPolicy Bypass -File "%ROOT%\Tools\GenerateBuildIdentity.ps1" -Root "%ROOT%" -Configuration "Release" -Milestone "29J.2"
...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  "$p='%ENGINE%'; $s=[Text.Encoding]::ASCII.GetString([IO.File]::ReadAllBytes($p)); if($s.Contains('29J.2') -and $s.Contains('LoadStaticBoxScene') -and $s.Contains('Player Scene Collision')){exit 0}else{exit 7}"
if errorlevel 1 (
    echo ERROR: Fresh executable does not contain the Step 29J.2 Player Scene markers.
    pause
    exit /b 1
)

>"%LOG%" echo RESULT=STEP_29J2_RELEASE_BUILD_SUCCEEDED
...
echo.
echo ============================================================
echo BUILD SUCCEEDED - Step 29J.2 is installed.
...
:layout_failed
echo.
echo ERROR: Step 29J.2 source/layout verification failed.
echo Re-extract HeritageEngine_Step_29J2.zip and allow file replacement.
pause
exit /b 1

:validation_failed
echo.
echo ERROR: Step 29J.2 project validation failed.
...
:build_failed
echo.
echo ERROR: Step 29J.2 build failed.
