...
set "LOG=%DIAGNOSTICS%\step29j_build_and_run.txt"
...
echo ============================================================
echo Heritage Engine - Step 29J Articulated Wheel Presentation
echo Native suspension + Ackermann steering + simulated wheel spin -> wheel meshes
...
for %%F in (
    ...
    echo ERROR: Required Step 29J file is missing:
    echo %%~F
    pause
    exit /b 1
)
...
findstr /C:"ARTICULATED WHEEL VISUALS - STEP 29J" "%MODULE%\Scripts\UI\Vehicle\Visual\WheelsPanel.lua" >nul || goto :layout_failed
findstr /C:"Vehicles/VisualWheels.lua" "%MODULE%\Scripts\Main.lua" >nul || goto :layout_failed

echo Generating Step 29J build identity...
powershell -NoProfile -ExecutionPolicy Bypass -File "%ROOT%\Tools\GenerateBuildIdentity.ps1" -Root "%ROOT%" -Configuration "Release" -Milestone "29J"
...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  "$p='%ENGINE%'; $s=[Text.Encoding]::ASCII.GetString([IO.File]::ReadAllBytes($p)); if($s.Contains('29J') -and $s.Contains('SetWheelTireModel') -and $s.Contains('GetWheelState')){exit 0}else{exit 7}"
if errorlevel 1 (
    echo ERROR: Fresh executable does not contain the Step 29J build identity plus existing native wheel-state markers.
    pause
    exit /b 1
)

>"%LOG%" echo RESULT=STEP_29J_RELEASE_BUILD_SUCCEEDED
...
echo.
echo ============================================================
echo BUILD SUCCEEDED - Step 29J is installed.
echo Your existing PlayerCar.obj was NOT supplied by this update, so the car you imported stays yours.
echo Go to VEHICLE ^> VISUAL ^> WHEELS to inspect the articulated wheel system.
echo Keep it OFF while PlayerCar.obj still contains rigid wheels, or enable it temporarily to see the native suspension/steer/spin presentation.
echo For the clean version, export the body without wheels and replace PlayerWheel.obj with an authored wheel centered at its origin.
...
:layout_failed
echo.
echo ERROR: Step 29J source/layout verification failed.
echo Re-extract HeritageEngine_Step_29J.zip and allow file replacement.
pause
exit /b 1

:validation_failed
echo.
echo ERROR: Step 29J project validation failed.
...
:build_failed
echo.
echo ERROR: Step 29J build failed.
