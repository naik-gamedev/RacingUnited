...
Check ($meshRendererFramePrep.Contains("scaleSafeLocalRotationOffset") -and $meshRendererFramePrep.Contains("T*R*Q*S") -and $meshRendererFramePrep.Contains("localRotationOffsetConsumed")) "VA02J embedded wheel-spin offsets compose before authored local scale/reflection so visible rim geometry rotates rigidly"
Check ($meshRendererFramePrep.Contains("semanticWheelTireNodeIndex") -and $meshRendererFramePrep.Contains("geometricWheelSpinDelta") -and $meshRendererFramePrep.Contains("axisAngleRotation")) "VA02K embedded wheel pivots use a geometry-derived rigid axle instead of trusting a slightly tilted authored Pivot basis"
Check ($meshRendererFramePrep.Contains("bindPivotCenter") -and $meshRendererFramePrep.Contains("transformPoint3(currentPivotGlobal, { 0.0f, 0.0f, 0.0f })") -and $meshRendererFramePrep.Contains("tire_vs_pivot_radial_mm")) "VA02L wheel spin keeps the authored Pivot origin as hub centre while tire geometry supplies only the axle direction"
...
$dynamicSurfaceGpuRuntimeHeader = ReadText (Join-Path $Root "Engine\HeritageEngine\Graphics\DynamicSurface\DynamicSurfaceGpuRuntime.hpp")
$dynamicSurfaceGpuResources = ReadText (Join-Path $Root "Engine\HeritageEngine\Graphics\DynamicSurface\DynamicSurfaceGpuResources.cpp")
$dynamicSurfaceGpuResidency = ReadText (Join-Path $Root "Engine\HeritageEngine\Graphics\DynamicSurface\DynamicSurfaceGpuResidency.cpp")
$entityMeshDynamicSurface = ReadText (Join-Path $Root "Engine\HeritageEngine\Graphics\Renderer\EntityMeshDynamicSurface.cpp")
$entityMeshWetness = ReadText (Join-Path $Root "Engine\HeritageEngine\Graphics\Renderer\EntityMeshSurfaceWetness.cpp")
$entityMeshShaders = ReadText (Join-Path $Root "Engine\HeritageEngine\Graphics\Renderer\EntityMeshShaders.hpp")
...
Check ($surfacePresentationRendererCpp.Contains("kTireMarkGpuDrawDistanceM = 500.0") -and $surfacePresentationRendererCpp.Contains("kTireMarkGpuDetailedDistanceM = 200.0") -and ($surfacePresentationRendererCpp.Contains("GL_POLYGON_OFFSET_FILL") -and $surfacePresentationRendererCpp.Contains("glPolygonOffset(-1.0f, -4.0f)")) -and $surfacePresentationRendererCpp.Contains("float ageOpacity = 1.0 - smooth01(ageT)") -and $surfacePresentationRendererCpp.Contains("float detailMix = nearWeight") -and $surfacePresentationRendererCpp.Contains("sampleCount = detailed ? 6 : 2") -and $surfacePresentationRendererCpp.Contains("distanceMeters / max(uDrawDistance") -and $surfacePresentationRendererCpp.Contains("uNearMaterialAuthority") -and $surfacePresentationRendererCpp.Contains("uWeatherWetness")) "LIVETRACK22 tire-mark far persistence LOD uses a full-range fade, wet-contrast suppression, near material handoff and non-flickering decal depth bias"
Check ($dynamicSurfaceGpuRuntimeHeader.Contains("StateRuntime m_tireMarks") -and $dynamicSurfaceGpuRuntimeHeader.Contains("syncTireMarkPresentation") -and $dynamicSurfaceGpuResources.Contains("rasterizeTireMarkTile") -and $dynamicSurfaceGpuResources.Contains("m_tireMarkSerialsByTile") -and $dynamicSurfaceGpuResidency.Contains("clearStateSlot(m_tireMarks") -and $entityMeshDynamicSurface.Contains("syncTireMarkPresentation") -and $entityMeshWetness.Contains("uGpuTireMarkAtlas") -and $entityMeshShaders.Contains("gpuTireMarkStrength") -and $entityMeshShaders.Contains("markRangeVisibility") -and $entityMeshShaders.Contains("wetVisibility")) "LIVETRACK22 tire marks share Dynamic Surface 10m/256x256 near tile residency/indirection and weather-aware road-material presentation"
